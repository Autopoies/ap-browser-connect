...
test("select matches option by value or label and fires change", () => {
	const options = [
	...
	assert.deepEqual(byValue.selected, { value: "b", label: "Beta" });
	assert.equal(changed, 2); // change + input

	const byLabel = performNativeSelect(document, "sel", rules, "Alpha");
	assert.equal(byLabel.status, "ok");
	assert.equal(target.selectedIndex, 0);
});

test("select reports available options when nothing matches", () => {
	const target = {
		tagName: "SELECT",
		options: [{ value: "a", textContent: "Alpha" }],
		matches: () => false,
		closest: () => null,
		scrollIntoView: () => {},
		focus: () => {},
	...
	const result = performNativeSelect(
		{ querySelector: () => target },
		"sel",
		interactionDenyRules([POLICY]),
		"nope",
	);
	assert.equal(result.status, "option_not_found");
	assert.deepEqual(result.available, [{ value: "a", label: "Alpha" }]);
});

test("select rejects non-select targets and denied targets", () => {
	const div = {
		tagName: "DIV",
		matches: () => false,
		closest: () => null,
	...
	assert.equal(
		performNativeSelect({ querySelector: () => div }, "x", [], "a").status,
		"not_a_select",
	);
	const denied = {
		tagName: "SELECT",
		matches: (s) => s === '[data-action="acknowledge-guidelines"]',
		closest: () => null,
		scrollIntoView: () => {},
		focus: () => {},
		options: [],
