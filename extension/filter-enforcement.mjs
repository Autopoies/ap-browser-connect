...
export function hasFilterDiagnostics(metadata) {
  return Boolean(
    metadata && FILTER_COUNTER_KEYS.some((key) => Number(metadata[key] || 0) > 0),
  );
}
...
export function matchingPolicies(policies, operatedUrl, method) {
  ...
  return policies.filter((policy) => {
    ...
    return !Array.isArray(match.methods)
...
function redactString(value, block) {
  ...
  const replacement = typeof block.replacement === "string"
    ? block.replacement
    : "[FILTERED]";
  let cursor = 0;
  let output = "";
...
// This function is serialized into Runtime.evaluate. Keep it self-contained
// except for emptyFilterMetadata, which the expression builder injects.
export function performGuardedInteraction(documentObject, targetSelector, rules, kind, value) {
  const metadata = emptyFilterMetadata();
...
export function buildInteractionExpression(kind, targetSelector, value, rules) {
  return `(() => {
    const emptyFilterMetadata = ${emptyFilterMetadata.toString()};
